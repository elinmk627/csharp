...
namespace BackgroundWorkertTest {
    /// <summary>
    ...
    public partial class MainWindow : Window
    {
        ...
        protected override void OnInitialized(EventArgs e) {
            ...
            myThread = new BackgroundWorker() {     //백그라운드 워커 초기화
                WorkerReportsProgress = true,               //작업의 진행율이 바뀔때 ProgressChanged 이벤트 발생여부
                WorkerSupportsCancellation = true        //작업취소 가능여부 true로 설정
            };

            //백그라운드에서 실행될 콜백 이벤트 생성            
            myThread.DoWork += myThread_DoWork;     //해야할 작업을 실행할 메소드 정의

            //UI쪽에 진행사항을 보여주기 위해 WorkerReportsProgress 속성값이 true 일때만 이벤트 발생
        ...
        private void btnStart_Click(object sener, RoutedEventArgs e) {
            int num;
